...
#
# Writes: dist/rollcagestageii-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix rollcagestageii \
  --exe-name Rollcage_Stage_II \
  --omit-openbios \
  --display-name "Rollcage Stage II" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Rollcage Stage II (USA) disc" \
  --runtime-dir mods \
